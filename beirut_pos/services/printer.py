...
_PERMISSION_HINT_EMITTED = False
...
def _emit_permission_guidance(path: str) -> None:
    global _PERMISSION_HINT_EMITTED
    if _PERMISSION_HINT_EMITTED:
        return

    current_user = getuser()
    _log(
        "💡 Permission issue detected while opening thermal printer device. "
        f"Ensure your user has write access to {path}."
    )
    _log(
        "   • Add the user to the 'lp' group (logout/login afterwards):"
        f" sudo usermod -a -G lp {current_user}"
    )
    _log(
        "   • Or create a udev rule such as:"
        " SUBSYSTEM==\"usb\", ATTR{idVendor}==\"0483\", ATTR{idProduct}==\"5743\", "
        "MODE=\"0664\", GROUP=\"lp\""
    )
    _log(
        "   • Reload udev rules: sudo udevadm control --reload && sudo udevadm trigger"
    )
    _PERMISSION_HINT_EMITTED = True


def _probe_device_access(path: str) -> bool:
    ...
    try:
        ...
        _log_printer_error(f"Permission denied for {path}", exc)
        _emit_permission_guidance(path)
        return False
    except OSError as exc:
        _log_printer_error(f"Unable to probe {path}", exc)
        return False
    else:
        os.close(fd)
        return True
...
def _try_file_printer():
    ...
    for path in lp_devices:
        if not _probe_device_access(path):
            continue
        try:
...
def _setup_printer_arabic(printer) -> bool:
    ...
    try:
        ...
        if isinstance(exc, PermissionError):
            _emit_permission_guidance(getattr(printer, "device", getattr(printer, "path", "")) or "device")
        return False
