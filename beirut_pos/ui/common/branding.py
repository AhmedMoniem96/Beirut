...
def build_login_stylesheet() -> str:
    ...
    parts.extend([
        "}",
        "#LoginCard {",
        "    background-color: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 rgba(28, 17, 11, 0.95), stop:1 rgba(12, 7, 4, 0.92));",
        "    border-radius: 36px;",
        f"    color: {text};",
        ...
        "#LoginCard QFrame#LoginForm { background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 rgba(22, 13, 8, 0.78), stop:1 rgba(10, 6, 4, 0.88)); border-radius: 28px; border: 1px solid rgba(255,255,255,0.12); }",
        f"#LoginDialog QPushButton {{ background-color: {accent}; color: #1B0F08; border-radius: 20px; padding: 15px 30px; font-weight: 700; letter-spacing: 0.6px; min-height: 52px; }}",
        ...
        f"#LoginDialog QFrame#DSFormField {{ background-color: rgba(255,255,255,0.09); border: 1px solid rgba(255,255,255,0.2); border-radius: 20px; }}",
        f"#LoginDialog QFrame#DSFormField[data-focused=\"true\"] {{ border-color: {accent}; background-color: rgba(255,255,255,0.18); }}",
        f"#LoginDialog QFrame#DSFormField[data-status=\"error\"] {{ border-color: #B24646; background-color: rgba(178,70,70,0.12); }}",
        f"#LoginDialog QFrame#DSFormField[data-status=\"success\"] {{ border-color: #2E7D54; background-color: rgba(46,125,84,0.12); }}",
        f"#LoginDialog QFrame#DSFormField QLabel[data-role=\"label\"] {{ color: {muted}; font-weight: 700; margin-bottom: 10px; }}",
        f"#LoginDialog QFrame#DSFormField QLabel[data-role=\"helper\"] {{ color: {muted}; margin-top: 4px; }}",
        f"#LoginDialog QLineEdit#DSTextField {{ border: 1px solid rgba(218, 190, 140, 0.55); border-radius: 18px; padding: 14px 24px; margin-top: 6px; min-height: 52px; background-color: rgba(255,255,255,0.97); color: #1A0F08; font-size: 15pt; font-weight: 600; letter-spacing: 0.5px; selection-background-color: {accent}; selection-color: #1A0F08; background-clip: padding; }}",
        f"#LoginDialog QLineEdit#DSTextField::placeholder {{ color: rgba(26, 15, 8, 0.42); }}",
        ...
        "#LoginDialog QLineEdit#DSTextField[data-status=\"error\"] { border: 2px solid #B24646; }",
        "#LoginDialog QLineEdit#DSTextField[data-status=\"success\"] { border: 2px solid #2E7D54; }",
        "#LoginDialog QLineEdit#DSTextField::clear-button { border: none; background: transparent; width: 18px; height: 18px; margin: 0 6px; }",
...
def build_main_window_stylesheet() -> str:
    ...
    parts.extend(
        [
            ...
            "QMainWindow QPushButton:focus { outline: 3px solid rgba(212,160,94,0.72); outline-offset: 2px; }",
            ...
            f"QMainWindow QLineEdit:focus, QMainWindow QComboBox:focus {{ border: 2px solid {accent}; }}",
