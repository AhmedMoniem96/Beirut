...
class PaymentPanel(QWidget):
    def __init__(self, on_pay, on_discount):
        super().__init__()
        v = QVBoxLayout(self)
        self.sub = QLabel("المجموع قبل الخصم: LE 0.00")
        self.disc = QLabel("الخصم: LE 0.00")
        self.total = QLabel("المجموع النهائي: LE 0.00")
    ...
    def set_totals(self, subtotal_cents, discount_cents, total_cents):
        self.sub.setText(f"المجموع قبل الخصم: {fmt_le(cents_to_le(subtotal_cents))}")
        self.disc.setText(f"الخصم: {fmt_le(cents_to_le(discount_cents))}")
        self.total.setText(f"المجموع النهائي: {fmt_le(cents_to_le(total_cents))}")
